...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=True, image_weights=False,
        ...
        self.img_files_rgb=[x.replace('gray/cam0', 'rgb/cam0').replace('gray/cam1', 'rgb/cam0') for x in self.img_files]

        # Define labels
        self.label_files = [x.replace('JPEGImages', 'labels').replace(os.path.splitext(x)[-1], '.txt')
                            for x in self.img_files]
        self.label_files_rgb = [x.replace('gray/cam0', 'rgb/cam0').replace('gray/cam1', 'rgb/cam0') for x in self.label_files]
        ...
        self.imgs = [None] * n
        self.imgs_rgb = [None] * n
        self.labels = [None] * n
        self.labels_rgb = [None] * n
        ...
        if cache_labels or image_weights:  # cache labels for faster training
            self.labels_rgb = [np.zeros((0, 5))] * n
            extract_bounding_boxes = False
            create_datasubset = False
            pbar = tqdm(self.label_files_rgb, desc='Reading labels')
            nm, nf, ne, ns = 0, 0, 0, 0  # number missing, number found, number empty, number datasubset
            for i, file in enumerate(pbar):
                try:
                    with open(file, 'r') as f:
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            img, labels , img_rgb, labels_rgb = load_mosaic(self, index)
            h, w, _ = img.shape
        ...
        if self.augment:
            ...
            img_rgb, labels_rgb = random_affine(img_rgb, labels_rgb,
                                        degrees=hyp['degrees'] * g,
                                        translate=hyp['translate'] * g,
                                        scale=hyp['scale'] * g,
                                        shear=hyp['shear'] * g)
            ...
            #    labels_rgb = cutout(img_rgb, labels_rgb)
        ...
        if nL_rgb:
            # convert xyxy to xywh
            labels_rgb[:, 1:5] = xyxy2xywh(labels_rgb[:, 1:5])

            # Normalize coordinates 0 - 1
            labels_rgb[:, [2, 4]] /= img_rgb.shape[0]  # height
            labels_rgb[:, [1, 3]] /= img_rgb.shape[1]  # width
        ...
        if nL_rgb:
            labels_out_rgb[:, 1:] = torch.from_numpy(labels_rgb)
        ...
        return torch.from_numpy(img), labels_out, torch.from_numpy(img_rgb), labels_out_rgb, img_path, (h, w)

    @staticmethod
    def collate_fn(batch):
        img, label,img_rgb,label_rgb, path, hw = list(zip(*batch))  # transposed
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        for i, l in enumerate(label_rgb):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0),torch.stack(img_rgb, 0), torch.cat(label_rgb, 0), path, hw
...
def load_image(self, index):
    ...
    return img
def load_image_rgb(self, index):
    #blur_size=[3,5]
    #blur=True
    # loads 1 image from dataset
    img = self.imgs_rgb[index]
    if img is None:
        img_path = self.img_files_rgb[index]
        img = cv2.imread(img_path)  # BGR
        assert img is not None, 'Image Not Found ' + img_path
        r = self.img_size / max(img.shape)  # size ratio
        if self.augment and r < 1:  # if training (NOT testing), downsize to inference shape
...
def load_mosaic(self, index):
    ...
    img4_rgb = np.zeros((s * 2, s * 2, 3), dtype=np.uint8) + 128  # base image with 4 tiles
    ...
    for i, index in enumerate(indices):
        ...
        img4_rgb[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
        padw = x1a - x1b
        padh = y1a - y1b

        # Load labels
        label_path = self.label_files_rgb[index]
        if os.path.isfile(label_path):
            ...
            if len(labels_singal) == 0:
                labels4_rgb.append(np.zeros((0, 5), dtype=np.float32))
            else:
                labels4_rgb.append(labels_singal)
    labels4_rgb = np.concatenate(labels4_rgb, 0)


    if random.random()<0.35:
       hyp = self.hyp
       img4, labels4,a_rgb ,s_rgb  = random_affine_rgb(img4, labels4,
                                  degrees=hyp['degrees'],
                                  translate=hyp['translate'],
                                  scale=hyp['scale'],
                                  shear=hyp['shear'],a_r=None,s_r=None)
       img4_rgb, labels4_rgb,a_rgb1,s_rgb1 = random_affine_rgb(img4_rgb, labels4_rgb,
                                  degrees=hyp['degrees'],
                                  translate=hyp['translate'],
                                  scale=hyp['scale'],
                                  shear=hyp['shear'],a_r=a_rgb,s_r=s_rgb)

    # Center crop
    a = s // 2
    img4 = img4[a:a + s, a:a + s]
    img4_rgb = img4_rgb[a:a + s, a:a + s]
    labels4[:, 1:] -= a
    labels4_rgb[:, 1:] -= a

    return img4, labels4,img4_rgb,labels4_rgb
...
def random_affine_rgb(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10,a_r=None,s_r=None):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    # https://medium.com/uruvideo/dataset-augmentation-with-random-homographies-a8f4b44830d4
...
def cutout(image, labels):
    ...
    # create random masks
    scales = [0.1] * 1  # + [0.25] * 4 + [0.125] * 16 + [0.0625] * 64 + [0.03125] * 256  # image size fraction
    for s in scales:
